...
#ifndef SMM_SERVICES_LIB_H_
#define SMM_SERVICES_LIB_H_

// ASSERT_SMM_PROTOCOL_ALREADY_INSTALLED
/** Macro that calls DebugAssert() if a protocol is already installed in the
  ...
  If Handle is NULL, then a check is made to see if the protocol specified by
  Guid is present on any handle in the handle database.  If Handle is not NULL,
  then a check is made to see if the protocol specified by Guid is present on
  the handle specified by Handle.  If the check finds the protocol, then
  DebugAssert() is called passing in the source filename, source line number,
  and Guid.

  If Guid is NULL, then ASSERT().

  @param[in] Handle  The handle to check for the protocol.  This is an optional
                     parameter that may be NULL.  If it is NULL, then the
                     entirehandle database is searched.

  @param[in] Guid    The pointer to a protocol GUID.
**/
#if !defined(MDEPKG_NDEBUG)
  #define ASSERT_SMM_PROTOCOL_ALREADY_INSTALLED(Handle, Guid)                 \
    ...
    do {                                                                      \
      if (DebugAssertEnabled ()) {                                            \
        if ((Handle) == NULL) {                                               \
          if (!EFI_ERROR (                                                    \
                 SmmLocateProtocol ((Guid), NULL, &AssertInterface)           \
                 )) {                                                         \
            _ASSERT (Guid already installed in database);                     \
          }                                                                   \
        } else if (!EFI_ERROR (                                               \
                      SmmHandleProtocol ((Handle), (Guid), &AssertInterface)  \
                      )) {                                                    \
