...
#define OVERRIDE_IDENTIFIER_FROM_LIST_ENTRY(ListEntry)  \
  CR (                                                  \
    ...
    This,                                               \
    OVERRIDE_IDENTIFIER_SIGNATURE                       \
    )

#define OVERRIDE_IDENTIFIER_FUNCTION (Identfier, Type)  \
  ((Type)OVERRIDE_IDENTIFIER_FROM_LIST_ENTRY (          \
           (Identifier)->This.BackLink                  \
           )->Function)
/// @}
  ...
  )
{
  ...
  Identifier             = AllocatePool (sizeof (*Identifier));
  ...
  )
{
  ...
  *Override->Destination = (OVERRIDE_IDENTIFIER_FROM_LIST_ENTRY (
                              Identifier->This.BackLink
                              ))->Function;
  ...
  )
{
  ...
  if (!IsListEmpty (&Override->Identifiers)) {
    Identifier = OVERRIDE_IDENTIFIER_FROM_LIST_ENTRY (
                   Override->Identifiers.ForwardLink
                   );

    do {
      NextIdentifier = OVERRIDE_IDENTIFIER_FROM_LIST_ENTRY (
                         Identifier->This.ForwardLink
                         );
