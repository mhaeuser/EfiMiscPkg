  ...
  )
{
  ...
  ASSERT (((AgentHandle != NULL) && (ControllerHandle != NULL)) || (Attributes != (EFI_OPEN_PROTOCOL_BY_CHILD_CONTROLLER | EFI_OPEN_PROTOCOL_EXCLUSIVE)));
  ...
  )
{
  ...
  DEBUG_CODE (
    if ((Status != EFI_NOT_FOUND) && (Status != EFI_SECURITY_VIOLATION)) {
      ASSERT_EFI_ERROR (Status);
    }
  );
