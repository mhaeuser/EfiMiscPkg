...
#include <Library/EfiImageLib/EfiImageLib.h>
  ...
  )
{
  ...
  DEBUG_CODE (
    if ((Status != EFI_UNSUPPORTED) && (Status != EFI_OUT_OF_RESOURCES) && (Status != EFI_LOAD_ERROR)
     && (Status != EFI_DEVICE_ERROR) && (Status != EFI_ACCESS_DENIED)) {
  ...
  )
{
  ...
  // don't assert on EFI_INVALID_PARAMETER as ExitBootServices might get passed an outdated memory map, which gets updates as part of the control flow.
